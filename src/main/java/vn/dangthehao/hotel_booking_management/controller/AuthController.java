...
public class AuthController {
  ...
  public ResponseEntity<ApiResponse<AuthResponse>> refreshToken(
    ...
    ApiResponse<AuthResponse> apiResponse =
        authenticationService.renewAccessAndRefreshToken(refreshToken);

    return ResponseEntity.status(HttpStatus.OK).body(apiResponse);
  }

  @PostMapping("/passwords/resets")
  public ResponseEntity<ApiResponse<Void>> sendOTP(@RequestBody CheckEmailRequest request) {
  ...
  public ResponseEntity<ApiResponse<Void>> resetPassword(
    ...
    return ResponseEntity.status(HttpStatus.OK)
        .body(authenticationService.resetPassword(request, resetToken));
  }
}
